...
I researched [Company] and know you're a leader in the [Industry/Niche]. Given the [Complexity 1] and [Complexity 2] you face as a [Job Title], you're most likely focused on [Positive Goal 1] and [Positive Goal 2]. A great starting point to help is an offshore [Role 1] to [Task 1] or a [Role 2] to [Task 2]. Aside from the roles I've mentioned, what type of role do you think might also be suitable for offshore?`
  ...
  function fmtMtgDate(raw: string): string {
    if (!raw) return '[Date]'
  ...
  function buildSubject(): string {
    return `Discovery Call Confirmed — ${leadName || '[Lead Name]'} × Outsource Accelerator | ${mtgDate ? fmtMtgDate(mtgDate) : '[Date]'}`
  }

  function buildEmailHtml(): string {
    const ln = leadName || '[Lead Name]'
    const bdr = yourName || '[BDR Name]'
    const dateStr = mtgDate ? fmtMtgDate(mtgDate) : '[Date]'
    const timeStr = mtgTime || '[Time]'
    const dur = mtgDuration
    const linkHtml = mtgLink
      ? `<a href="${mtgLink}" style="color:#1A2DB5;">${mtgLink}</a>`
      : '[Meeting Link]'
    return `<div style="font-family:Arial,sans-serif;max-width:560px;color:#222;line-height:1.6;">
<p>Hi ${ln},</p>
<p>Great speaking with you! Just confirming — our Discovery Call is all set. Here are the details:</p>
<table style="margin:16px 0;border-collapse:collapse;">
  <tr><td style="padding:5px 14px 5px 0;font-weight:700;">📅 Date &amp; Time</td><td style="padding:5px 0;">${dateStr} at ${timeStr}</td></tr>
  <tr><td style="padding:5px 14px 5px 0;font-weight:700;">⏱ Duration</td><td style="padding:5px 0;">~${dur} minutes</td></tr>
  <tr><td style="padding:5px 14px 5px 0;font-weight:700;">🔗 Meeting Link</td><td style="padding:5px 0;">${linkHtml}</td></tr>
</table>
<p>In this call we'll cover:</p>
<ul style="margin:8px 0;padding-left:20px;">
  <li>How Outsource Accelerator helps companies build offshore teams in the Philippines</li>
  <li>Your current team structure and what roles could make sense to offshore</li>
  <li>Next steps if there's a fit</li>
</ul>
<p>Please <strong>accept the calendar invite</strong> when it arrives — it locks in the time on both ends.</p>
<p>If anything comes up or you need to reschedule, just reply and I'll sort it out.</p>
<p>Looking forward to speaking with you!</p>
<p style="margin-top:20px;">${bdr}<br><span style="color:#666;">Outsource Accelerator</span></p>
</div>`
  }

  function buildEmailPlain(): string {
    const ln = leadName || '[Lead Name]'
    const bdr = yourName || '[BDR Name]'
    const dateStr = mtgDate ? fmtMtgDate(mtgDate) : '[Date]'
    const timeStr = mtgTime || '[Time]'
    const linkStr = mtgLink || '[Meeting Link]'
    const dur = mtgDuration
    return `Hi ${ln},

Great speaking with you! Just confirming — our Discovery Call is all set. Here are the details:

📅 Date & Time: ${dateStr} at ${timeStr}
⏱ Duration: ~${dur} minutes
🔗 Meeting Link: ${linkStr}

In this call we'll cover:
- How Outsource Accelerator helps companies build offshore teams in the Philippines
- Your current team structure and what roles could make sense to offshore
- Next steps if there's a fit

Please accept the calendar invite when it arrives — it locks in the time on both ends.

If anything comes up or you need to reschedule, just reply and I'll sort it out.
...
Outsource Accelerator`
  ...
  async function copyEmailBody() {
    try {
      await navigator.clipboard.write([
        new ClipboardItem({
          'text/html': new Blob([buildEmailHtml()], { type: 'text/html' }),
          'text/plain': new Blob([buildEmailPlain()], { type: 'text/plain' }),
        })
      ])
    } catch {
      await navigator.clipboard.writeText(buildEmailPlain())
    }
    setEmailCopied(true)
  ...
  async function copySubject() {
    ...
    setSubjCopied(true)
    setTimeout(() => setSubjCopied(false), 2000)
  }
  ...
  return (
    <div className="call-screen">
      ...
      {/* ── Booking Confirmation Email Panel ── */}
      {showEmail && (
        <div className="reference-bar">
          <div className="reference-bar-header">
            <span className="reference-bar-title">Booking Confirmation Email</span>
            <button className="btn-ref-close" onClick={() => setShowEmail(false)}>Close</button>
          </div>
          <div className="email-fields-row">
            <label className="email-field-wrap">
              <span className="email-field-lbl">Date</span>
              <input type="date" className="email-field-input" value={mtgDate} onChange={e => setMtgDate(e.target.value)} />
            </label>
            <label className="email-field-wrap">
              <span className="email-field-lbl">Time</span>
              <input type="text" className="email-field-input" placeholder="e.g. 10:00 AM PST" value={mtgTime} onChange={e => setMtgTime(e.target.value)} />
            </label>
            <label className="email-field-wrap">
              <span className="email-field-lbl">Duration</span>
              <select className="email-field-input" value={mtgDuration} onChange={e => setMtgDuration(e.target.value)}>
          ...
          <label className="email-field-wrap email-field-full">
            <span className="email-field-lbl">Meeting Link</span>
            <input type="text" className="email-field-input" placeholder="https://zoom.us/j/... or Google Meet URL" value={mtgLink} onChange={e => setMtgLink(e.target.value)} />
          </label>
          <div className="email-subject-row">
            <span className="email-subject-text">{buildSubject()}</span>
            <button className={`btn-subj-copy${subjCopied ? ' btn-copied' : ''}`} onClick={copySubject}>
              {subjCopied ? 'Copied!' : 'Copy Subject'}
            </button>
          </div>
          <div className="email-preview" dangerouslySetInnerHTML={{ __html: buildEmailHtml() }} />
          <button className={`btn-copy-email${emailCopied ? ' btn-copied' : ''}`} onClick={copyEmailBody}>
            {emailCopied ? '✓ Copied to clipboard!' : 'Copy Email Body'}
          </button>
        </div>
      )}
